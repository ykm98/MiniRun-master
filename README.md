...
首次打开小程序并触发云函数后，系统会自动写入一套演示数据，包括：

- 5 名模拟用户（不同院系/宿舍）
- 4 条校园通知公告
- 快递代取、代买、急事代办、陪替服务各若干条任务（含待接单、已接单、已完成等状态）
- 「关于我们」「联系我们」页面内容

**使用步骤（重要）：**

1. 在微信开发者工具中，右键 `cloudfunctions/mcloud` → **上传并部署：云端安装依赖**
2. 用微信开发者工具打开小程序，进入后台管理首页
3. 点击底部 **「导入演示数据」** 按钮（推荐，最可靠）
4. 后台管理登录：`admin` / `123456`

导入成功后，后台首页应显示：用户 5+、公告 4+、代取 4、急事 3、代买 3、陪替 3。

**重新写入演示数据：** 在后台管理首页再次点击「导入演示数据」即可。
